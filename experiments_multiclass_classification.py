...
from sklearn.metrics import f1_score,precision_recall_curve,confusion_matrix,precision_score,recall_score
...
from data_loaders import load_dataset,get_multiclass_datasets
...
xgb_search_space = {
    'objective': hp.choice('objective', ['multi:softmax']),  # Objective function for ranking
...
def try_model_cv(hyperparams,X,y,TreeEnsemble=RandomForestClassifier,WeightedClassifier=None,rf_params=None,rs=0,mrf_fixed=None):
    try:
        ...
        for fold, (train_index, val_index) in enumerate(skf.split(X, y)):
            ...
            val_losses.append(-f1_score(yf_val,prediction_val,average="macro"))
            train_losses.append(-f1_score(yf_train,prediction_train,average="macro"))
...
for dataset_name in names:
    ...
    mrf_fixed = {'gamma': 0.1, 'max_rate': None, 'rate':0.0, 'risk_aversion': 0.5}
    best_params = dict({})
    curves=dict([])
    #{'gamma': 1, 'max_rate': None, 'rate': None, 'risk_aversion': 1}
    for rs in range(0,20):
        ...
        performance = dict({"method":[],'accuracy':[],"f1_micro":[],"f1_macro":[],'elapsed':[]})
        ...
        for TreeEnsemble, WeightedClassifier,search_space in weighted_methods:
            ...
            if TreeEnsemble == XGBClassifier:
                ...
                if WeightedClassifier==MarkowitzForest or WeightedClassifier==IndependentVarianceForest:
                    ...
                    for fold, (train_index, val_index) in enumerate(skf.split(X_train, y_train)):
                        ...
                        weighted_clf.set_base_model(rf)
                        _=weighted_clf.estimate_weights_clf_multi(Xf_train.values,yf_train.values,Xf_val.values,yf_val.values,oob_estimate=True,rates=True)
                    ...
                    elapsed = (end - start)
                    predict_test = weighted_clf.weighted_predict_multi(X_test.values) 
                else:
                    weights_lst = []
                    models = []
                    skf = StratifiedKFold(n_splits=3,shuffle=True,random_state=rs)
            ...
            f1m = f1_score(y_test,predict_test,average="micro")
            f1M = f1_score(y_test,predict_test,average="macro")
            print(f"{methods[i]} {rs}: {f1m} {f1M}")
            print(prt)
            performance['method'].append(methods[i]) #
            performance['accuracy'].append(accs)
            performance['f1_micro'].append(f1m)
            performance['f1_macro'].append(f1M)
    ...
    tt.to_csv(f"results_multiclass_classification/{dataset_name}_run.csv",index=False)
